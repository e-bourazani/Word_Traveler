...
N_WORDS = 10
ARROW_PREFIX = "Κατεύθυνση"
CLUES_PREFIX = "Στοιχεία"
INTENDED_DESTINATION_PREFIX = "Προορισμός"
CARDINAL_DIRECTIONS = "Βόρεια", "Νότια", "Ανατολικά", "Δυτικά"
NEGATION_PREFIX = "όχι"
POINT_PREFIX = "Τοποθεσία"
START_LOCATION = 1
N_TARGET_LOCATIONS = 6
N_TARGET_SPECIAL_LOCATIONS = 3
N_DIMENSIONS = "5x5"  # number of dimensions in the city map, currently only 3x3 maps are supported 
DIMENSION = 5
LANGUAGE = "el"  # language of the game, greek
    ...
    def on_generate(self):
        ...
        for exp_name, experiment in experiment.items():   # build n instances for each experiment
            for game_id in range(N_INSTANCES):
                ...
                with open('resources_el/maps_5x5_el.json', 'r') as f:
                    cities = json.load(f)
                ...
                """ number of arrows """
                arrow_dict = {
                    '3': ['three_clues_strict_el', 'three_clues_nonstrict_el'],
                    '2': ['two_clues_strict_el', 'two_clues_nonstrict_el'],
                    '1': ['one_clues_strict_el', 'one_clues_nonstrict_el']
                }
                ...
                difficulty_dic = {
                    'nonstrict':['three_clues_nonstrict_el', 'two_clues_nonstrict_el', 'one_clues_nonstrict_el'],
                    'strict': ['three_clues_strict_el', 'two_clues_strict_el', 'one_clues_strict_el'],
                }
                ...
                """ word lists """
                adjectives = self.load_file('resources_el/wordlist_el.txt').strip('\n').split('\n')
                ...
                instance['prompt_player_a'] = self.create_prompt(self.load_template('resources_el/prompts/traveler_prompt_el.template'), N_DIMENSIONS, ARROW_PREFIX, CLUES_PREFIX, INTENDED_DESTINATION_PREFIX, CARDINAL_DIRECTIONS, NEGATION_PREFIX, None, city_name, string_city_map, n_arrows, string_starting_location, string_target_location, string_special_target_locations, word_list)
                instance['prompt_player_b'] = self.create_prompt(self.load_template('resources_el/prompts/local_prompt_el.template'), None, None, None, None, None, None,POINT_PREFIX, city_name, string_city_map, None, string_starting_location, None, None,None)
                errors_file = os.path.join('resources_el', 'errors_el.json')
